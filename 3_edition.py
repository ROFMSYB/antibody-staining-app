...
if __name__ == '__main__':
    input_path = 'C:\\Users\\ROFMSYB\\Desktop\\staining_plan.xlsx'
    output_path = 'C:\\Users\\ROFMSYB\\Desktop\\staining_result.xlsx'
    sample_n = 51

    df = load_excel_staining_plan(input_path)
    results = {}

    # 一抗
    df_primary = df[df['抗体类型'] == '一抗'].copy()
    df_primary_adjusted = adjust_fmo_generic(df, df_primary)
    results['一抗'] = compute_staining(df_primary_adjusted, df_primary, sample_n)

    # 二抗
    df_secondary = df[df['抗体类型'] == '二抗'].copy()
    if not df_secondary.empty:
        df_secondary_adjusted = adjust_fmo_generic(df, df_secondary)
        results['二抗'] = compute_staining(df_secondary_adjusted, df_secondary, sample_n)

    # 胞内抗体
    df_ic = df[df['抗体类型'] == '胞内'].copy()
    if not df_ic.empty:
        df_ic_adjusted = adjust_fmo_generic(df, df_ic)
        results['胞内抗体'] = compute_staining(df_ic_adjusted, df_ic, sample_n)
